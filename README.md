...
* **Оркестрация и Графы:** LangGraph (Message Reducers, Command-based Routing).
* **Векторный RAG-индекс:** Qdrant (Docker).
* **Гибридный поиск:** FastEmbed (Локальные эмбеддинги: плотные векторы `MiniLM` + разреженные векторы `SPLADE` для точного полнотекстового совпадения ключевых слов).
* **Реляционная СУБД:** PostgreSQL 16 (Docker-контейнер с изоляцией томов и SELinux флагами прав доступа).
* **Языковые модели:** Интеграция любых LLM (OpenAI, DeepSeek, Anthropic) через OpenRouter API и нативный пакет `langchain-openrouter` с поддержкой стабильного режима **Structured Outputs**.
...
* **UI/UX:** **Next.js** (Возможность загрузки любого количества проектов для параллельной работы агента).
